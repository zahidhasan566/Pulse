<template>
  <div class="container-fluid">
    <breadcrumb :options="['Services']">
      <button class="btn btn-primary" @click="addPartnerModal()">Add Service</button>
    </breadcrumb>
    ...
    <advanced-datatable :options="tableOptions">
      ...
      <template slot="action" slot-scope="row">
          ...
          <button style="font-size: 12px;width:65px;padding: 2px 0px"  v-if="isAdmin" type="button" @click="deletePartner(row.item.ServiceID)" class="btn btn-danger btn-sm"><i class="mdi mdi-trash-can"></i> Delete</button>

      </template>
    </advanced-datatable>

    <add-edit-service @changeStatus="changeStatus" v-if="loading"/>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      tableOptions: {
        source: 'service/list',  // Updated to grouped route
        search: true,
        slots: [3], // Logo and Action columns
  ...
  methods: {
    ...
    deletePartner(id) {
      ...
      this.deleteAlert(() => {
        this.axiosDelete('service/delete', id, (response) => {  // Updated to grouped route
