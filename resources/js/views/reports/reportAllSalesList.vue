<template>
  <div class="container-fluid">
    <breadcrumb :options="['All Sales Information Report']">
                ...
                <div class="table-responsive">
                  <table class="table table-bordered table-striped dt-responsive nowrap dataTable no-footer dtr-inline table-sm small">
                    ...
                    <tr v-for="(item, index) in contents" >
                        <td v-for="(item2, headerIndex) in headers.slice(1)" v-bind:class="isInt(item[item2]) === true ? 'text-right' : '' " v-if="index !== 32">
                            <span v-if="headerIndex ===11">
                                     <img v-if="item.ProfilePicture" height="40" width="40"
                                          :src="`${mainOrigin}public/storage/${item.ProfilePicture}`"
                                          alt="">
...
export default {
  ...
  methods: {
    filterAllReport(){
      this.isLoading = true
      this.form.Export = '';
      this.form.post(baseurl + "api/reports/all-sales-information", this.config()).then(response => {
    ...
    exportReport(){
      ...
      exportReportCSV(){
          ...
          this.form.post(baseurl + "api/reports/all-sales-information", this.config()).then(response => {
              ...
              link.href = url;
              link.setAttribute('download', 'all_sales_information_data.csv');
